...
TELEGRAM_BOT_TOKEN = '6098885239:AAHVBkq7CJOhZdL_Qp_Xe6Ny2zEriD8DAes'
...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        ...
        "*💬 Owner: @HACK_CHIYE*\n"
        "*📢 Channel: [TABISH OFFICIAL Channel](https://t.me/+SCgV7yRZK3Q3YTA1)*"
    )
...
async def help_command(update: Update, context: CallbackContext):
    chat_id = update.effective_chat.id
    message = (
        "*❓ How to Use the TABISH DDOS Bot?*\n\n"
        "*🔑 Commands:*\n\n"
        ...
        "*3. Contact the bot owner @HACK_CHIYE for assistance or clarifications.*\n\n"
        "*⚡ Owner: @HACK_CHIYE*\n"
        "*📢 Channel: [TABISH OFFICIAL Channel](https://t.me/+SCgV7yRZK3Q3YTA1)*"
    )
    await context.bot.send_message(chat_id=chat_id, text=message, parse_mode='Markdown')
...
async def approve(update: Update, context: CallbackContext):
    ...
    if chat_id != ADMIN_USER_ID:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ You need to get permission to use this command. Contact @HACK_CHIYE.*", parse_mode='Markdown')
        return
...
async def attack(update: Update, context: CallbackContext):
    ...
    if user_id not in users:
        await context.bot.send_message(chat_id=chat_id, text="*🤡 You need to get permission to use this bot. Contact owner @HACK_CHIYE.*", parse_mode='Markdown')
        return
